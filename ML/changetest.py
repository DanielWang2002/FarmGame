...
class AutoGame(Game):
    ...
    def __init__(self):
        ...
        self.csv_filename = f"dirtcost/game_{self.game_start_datetime.strftime('%Y%m%d_%H%M%S')}.csv"

        # 確認是否有log資料夾，若無則建立
        Path("dirtcost").mkdir(exist_ok=True)
    ...
    def run(self):
        ...
        while True:
            ...
            if current_ticks >= self.next_action_time:
                ...
                # 設定下一次動作的時間點 0.5 秒後）
                self.next_action_time = current_ticks + 100
